...
function linkInstall (pkgs, cb) {
  asyncMap(pkgs, function (pkg, cb) {
    ...
    log.info('msys2', 'npm.globalDir: %j', npm.globalDir)
    function n (er, data) {

      log.info('msys2', 'experiment1 %j', path.resolve('C:\\Users\\cchamberlain\\local\\npm\\node_modules\\@tixinc\\config', '..'))
      log.info('msys2', 'experiment2 %j', path.resolve('C:\\Users\\cchamberlain\\local\\npm\\node_modules\\@tixinc\\config', '..', '..'))

      if (er) return cb(er, data)
      // we want the ONE thing that was installed into the global dir
      var installed = data.filter(function (info) {
        var folder = info[1]
        log.info('msys2', 'folder: %j', folder)
        // this was causing a bug on msys2 because it needed to go up two levels to match on resolved when running npm link with private modules
        var resolved = path.resolve(folder, '..');
        if(resolved.indexOf('@') !== -1) {
          resolved = path.resolve(resolved, '..');
        }
        log.info('msys2', 'resolved: %j', resolved)
        return resolved === npm.globalDir
      })
      log.info('msys2', 'installed: %j', installed)
